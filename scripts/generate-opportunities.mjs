...
const outputPath = path.resolve(__dirname, "../server/opportunities.ts");

const csvRaw = fs.readFileSync(csvPath, "utf8").replace(/^\uFEFF/, "");
const rows = parseCsv(csvRaw);

if (!rows.length) {
  throw new Error("CSV file had no rows");
}
...
const fileContents = `import { Opportunity } from "@shared/schema";
...
function convertRecord(record, index) {
  const id = record.ID?.trim() || String(index + 1);

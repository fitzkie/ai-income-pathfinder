...
const csvPath = path.resolve("private/213-side-hustles-fullNew .csv");
...
const targetIds = new Set(["204","205","206","207","208","209","210","211","212","213"]);

const csvRaw = fs.readFileSync(csvPath, "utf8").replace(/^\uFEFF/, "");
const rows = parseCsv(csvRaw);
const headerIndex = rows.findIndex((row) => (row[0] ?? "").trim() === "ID");
if (headerIndex === -1) {
  console.error("❌ CSV header row with ID column not found.");
  process.exit(1);
}
...
const records = rows
  ...
  .filter((record) => targetIds.has(record.ID?.trim()));

const opportunities = records.map((record) => convertRecord(record));
...
try {
  ...
  console.log(`✅ Seeded ${opportunities.length} opportunities (204-213).`);
} catch (error) {
...
function convertRecord(record) {
  ...
  const scoringRaw =
    record["Scoring Factors"] ?? record["Scoring Factors (What Makes This Rank Higher)"];
  const providedFactors = splitDetailedList(scoringRaw);
...
function generateScoringFactors(title, difficulty, timeToCash, startupCost, revenue, tags, skills) {
  ...
  if (difficulty <= 2) {
    factors.push(
      `${title} scores ${difficulty}/5 on difficulty, so operators with ${formatList(
        skills,
        "generalist"
      )} skills can ramp quickly.`
    );
  } else if (difficulty >= 4) {
    factors.push(
      `Higher difficulty (${difficulty}/5) keeps ${title.toLowerCase()} defensible and lets specialists charge premium retainers.`
    );
  } else {
    factors.push(
      `Moderate difficulty (${difficulty}/5) balances accessibility with room to differentiate on quality for ${title.toLowerCase()}.`
    );
  }

  if (timeToCash <= 2) {
    factors.push(
      `Fast time-to-cash rating (${timeToCash}/5) means you can validate ${title.toLowerCase()} in a few weeks.`
    );
  } else if (timeToCash >= 4) {
    factors.push(
      `Time-to-cash score of ${timeToCash}/5 rewards creators who can nurture longer sales cycles for outsized paydays.`
    );
  } else {
    factors.push(
      `Steady time-to-cash (${timeToCash}/5) provides predictable revenue once the pipeline for ${title.toLowerCase()} is live.`
    );
  }

  if (startupCost <= 2) {
    factors.push(
      `Minimal startup cost (${startupCost}/5) keeps downside low, so you can experiment with ${title.toLowerCase()} quickly.`
    );
  } else {
    factors.push(
      `Startup cost of ${startupCost}/5 acts as a barrier, filtering competitors and positioning ${title.toLowerCase()} as a premium offer.`
    );
  }
